...
def print_death_age(people_dict):
    ...
    for item in people_dict.items():
        ...
        print(
            f'id {key} name: {item_value[NAME_INDEX]} '
            f'age: {item_value[DEATH_YEAR_INDEX] - item_value[BIRTH_YEAR_INDEX]}'
        )
...
def count_genders(people_dict):
    ...
    for item in people_dict.items():
        item_value = item[1]
        if item_value[GENDER_INDEX] == 'F':
            females += 1
        if item_value[GENDER_INDEX] == 'M':
            males += 1

    print(f'total of males is {males}')
    print(f'total of females is {females}')
...
def print_marriages(marriages_dict, people_dict):
    ...
    for marriage in marriages_dict.items():
        marriage_item_key = marriage[0]
        marriage_item = marriage[1]

        husband = marriage_item[HUSBAND_KEY_INDEX]
        wife = marriage_item[WIFE_KEY_INDEX]

        statement = ''
        if husband in people_dict:
            value = people_dict[husband]

            wedding_age = marriage_item[WEDDING_YEAR_INDEX] - value[BIRTH_YEAR_INDEX]
            statement += f'{value[NAME_INDEX]} {wedding_age} > {marriage_item[WEDDING_YEAR_INDEX]} < '

        if wife in people_dict:
            value = people_dict[wife]

            wedding_age = marriage_item[WEDDING_YEAR_INDEX] - value[BIRTH_YEAR_INDEX]
            statement += f'{value[NAME_INDEX]} {wedding_age}'

        print(f'{marriage_item_key} {statement}')
